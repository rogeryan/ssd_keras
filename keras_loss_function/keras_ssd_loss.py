...
class SSDLoss:
    ...
    def log_loss(self, y_true, y_pred):
        ...
        log_loss = -tf.reduce_sum(y_true * tf.keras.backend.log(y_pred), axis=-1)
        return log_loss
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        n_negative_keep = tf.math.minimum(tf.math.maximum(self.neg_pos_ratio * tf.cast(n_positive,tf.int32), self.n_neg_min), n_neg_losses)
        ...
        def f2():
            ...
            negatives_keep = tf.cast(tf.reshape(
                negatives_keep, [batch_size, n_boxes]), tf.float32)
